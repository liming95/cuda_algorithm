...
void matmul_##TF##_tiling_register_bench(nvbench::state &state) {                       \
    matmul_tiling_register_bench<TF>(state);                                            \
}                                                                                       \
NVBENCH_BENCH(matmul_##TF##_tiling_register_bench)                                      \
...
__global__ void matmul_kernel_tiling_register(float* A, float* B, float* C, int N) {
    ...
    float* C_shared = B_shared + tile_dim * tile_dim;
...
template<int TILE_FACTOR>
void matmul_tiling_register_bench(nvbench::state &state){
    ...
    size_t shared_mem_size = 3 * tile_dim * tile_dim * sizeof(float);
    ...
    state.exec(nvbench::exec_tag::timer, [blocksPerGrid,
        ...
        matrix_dim
            ...
            timer.start();
            matmul_kernel_tiling_register<TILE_FACTOR><<<blocksPerGrid, threadsPerBlock, shared_mem_size,
