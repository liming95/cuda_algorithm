...
__global__ void matmul_kernel_tiling_bank(float* A, float* B, float* C, int N) {
    ...
    for(int i = 0; i < N / tile_dim; i++){
        ...
        B_shared[threadIdx.y*tile_dim+threadIdx.x^threadIdx.y] = B[(i*tile_dim+threadIdx.y)*N+col];
        __syncthreads();

        for(int j = 0; j < tile_dim; j++){
            sum += A_shared[threadIdx.y*tile_dim+j] * B_shared[j*tile_dim+threadIdx.x^j];
        }
        __syncthreads();
    }
...
void matmul_bank_bench(nvbench::state &state){
    ...
    state.exec(nvbench::exec_tag::timer, [blocksPerGrid,
        ...
        matrix_dim
            ...
            if (!isValidLaunchConfig(blocksPerGrid, threadsPerBlock)) {
                ...
                timer.start();
                matmul_kernel_tiling_bank<<<blocksPerGrid, threadsPerBlock, shared_mem_size,
...
NVBENCH_BENCH(matmul_bank_bench)
