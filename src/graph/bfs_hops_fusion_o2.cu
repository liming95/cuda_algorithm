...
__device__ int vf_num, block_offset;
//producer_num, processed_num;
...
__global__ void bfs_hops_fusion_2_o2(int* g_offset, int* g_edges, int node_num, int edge_num,
    ...
    __shared__ int blk_vertex_frontiers[BLOCK_MAX_SIZE];   // Todo: blk_vertex_frontiers  depends of shared memory
    ...
    // Second Level
    // 1. consume the frontiers in block shared memory
    hop += 1;
    ...
    if(blk_vf_num_rmd) {
      if(blk_tid < blk_vf_num_rmd) {
        ...
        neighbors_num[0] = end - start;
        blk_start_offset[blk_tid] = start;
        DEBUG_PRINT("(2.2r)vertex: %d, ngh_nums: %d\n", vertex, neighbors_num[0]);
      }
      else {
        neighbors_num[0] = 0;
      }
      ...
      for(int i = blk_tid; i < total_edges_per_block; i += block_size){
        ...
        ngb_hop = atomicMin(&distance[neighbor], hop);
        DEBUG_PRINT("(2.2r)neighbors(%d, %d), pre_hop: %d, hop: %d\n", vertex, neighbor, ngb_hop, hop);
        // produce frontiers for next step
        if(hop < ngb_hop) {
          int index = atomicAdd(output_num, 1);
          output_frontiers[index] = neighbor;
...
std::vector<int> test_bfs_hops_fusion_o2(std::vector<int> offset, std::vector<int> edges, int source){
    // build queue
    int * queue_in;
    // * queue_out;
    ...
    std::cout << "GPU: bfs_hops_fusion_o2. Elapsed time :" << milliseconds << " (ms)\n";
