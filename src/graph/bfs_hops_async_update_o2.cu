...
__global__ void initial_output_bitmap_o2(int* output_num, int update_num){
...
__global__ void update_node_status_o2(int* input_frontiers, int offset, int input_num, int* hops, int hop){
    int glb_tid = blockIdx.x * blockDim.x + threadIdx.x;
...
void bfs_hops_async_o2(std::vector<int> offset, std::vector<int> edges, int node_num, int edge_num,
    ...
    while(input_num){
        ...
        initial_output_bitmap_o2<<<1, 32>>>(d_output_num, input_num);
        // cudaError_t err = cudaGetLastError();
        // if (err != cudaSuccess) {
        //     printf("Kernel launch 1 error: %s\n", cudaGetErrorString(err));
        // }
        calculate_kernel_config(input_num, threadsPerBlock, blocksPerGrid);
        get_output_frontiers_o2<<<blocksPerGrid, threadsPerBlock>>>(d_offset, d_edges, node_num, edge_num,
        ...
        update_node_status_o2<<<blocksPerGrid, threadsPerBlock, 0, stream_update>>>(d_update_frontiers, update_offset, output_num,
                                                                   d_hops, cur_hop);
