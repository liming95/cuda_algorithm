...
__global__ void updata_node_status(int* node_frontiers, int nf_num, int* hops, int* visited_bitmap, int hop);
...
__global__ void get_edge_frontiers(int* offset, int* edges, int node_num, int edge_num,
    ...
    if(tid < *nf_num){
        ...
        for(int i = edge_start; i < edge_end; i++){
            ...
            int index = atomicAdd(ef_num, 1);
            edge_frontiers[index] = neighbor;

            // update edges_bitmap
            int offset_in_bitmap;
            GET_BIT_INDEX_OFFSET(neighbor, index, offset_in_bitmap);
            int bitmark = 1 << offset_in_bitmap;
            atomicOr(&edges_bitmap[index], bitmark);
        }
    }
}
...
__global__ void get_node_frontiers(int* edges_bitmap, int* visited_bitmap, int bitmap_len,
    ...
    if(tid < *ef_num){
        ...
        int visited = visited_bitmap[index] | edges_bitmap[index];
...
__global__ void updata_node_status(int* node_frontiers, int nf_num, int* hops, int* visited_bitmap, int hop){
    ...
    if(tid < nf_num){
        ...
        atomicOr(&visited_bitmap[index], bitmask);
    }
}
        ...
        for(int i = 0; i < LANE_NUM; i++){
            ...
            if(!has_node_frontier[prev] && has_edge_frontier[i]) {
                ...
                if(ef_num[i] != 0){
                    ...
                    get_node_frontiers<<<blocksPerGrid, threadsPerBlock, 0, stream>>>(d_edges_bitmap[prev], d_visited_bitmap, bitmap_len,
                                                                                    d_edge_frontiers[i], d_ef_num[i],
                    ...
                    if(nf_num_status != 0){
                        ...
                        updata_node_status<<<blocksPerGrid, threadsPerBlock, 0, stream>>>(d_node_frontiers_status[i], nf_num_status,
                                                                                    d_hops, d_visited_bitmap, cur_hop);
